...
translateID <-read_tsv("GAMC_VSGnome2Tb427_2018.translation.txt",col_names = F)
...
* the filters can be adjusted to include whatever subset of links
* the following code works best when the number of nodes plotted is < 1000
...
# rename patient VSG trinity IDs by cluster using the cluster reference table and filter so we only include expressed clusters
PtoNlink <- Nlinks[grep("TRINITY", Nlinks$to, value = F), ]
PtoNlink <- PtoNlink %>%
  inner_join(cluster_reference_table, by = c('to'='VSG')) %>%
  select(-to, to = cluster)
PtoNlink <- PtoNlink %>% select(from, to, evalue)
PtoNlink <- PtoNlink[PtoNlink$to %in% expressed, ]

filter_Nlinks <- Nlinks[-grep("TRINITY", Nlinks$to, value = F), ]
filter_Nlinks <- rbind(filter_Nlinks, PtoNlink)

PfromNlink <- filter_Nlinks[grep("TRINITY", filter_Nlinks$from, value = F), ]
PfromNlink <- PfromNlink %>%
  inner_join(cluster_reference_table, by = c('from'='VSG')) %>%
  select(-from, from = cluster)
PfromNlink <- PfromNlink %>% select(from, to, evalue)
PfromNlink <- PfromNlink[PfromNlink$from %in% expressed, ]

filter_Nlinks <- filter_Nlinks[-grep("TRINITY", filter_Nlinks$from, value = F), ]
filter_Nlinks <- rbind(filter_Nlinks, PfromNlink)

# check to make sure all expressed VSG are in table
# expect rhodesiense to be missing the 5 unknown-type clusters since they had undefined n-termini
unique(filter_Nlinks$from[grep("Gambiense", filter_Nlinks$from, value = F)])
unique(filter_Nlinks$from[grep("Rhodesiense", filter_Nlinks$from, value = F)])

unique(filter_Nlinks$to[grep("Gambiense", filter_Nlinks$to, value = F)])
unique(filter_Nlinks$to[grep("Rhodesiense", filter_Nlinks$to, value = F)])
...
# finish filtering using inner join
filter_NlinksFinal <- inner_join(filter_Nlinks, filter_ref[,1], by = c("to" = "original_seqID")) 
filter_NlinksFinal <- inner_join(filter_NlinksFinal, filter_ref[,1], by = c("from" = "original_seqID"))

length(unique(filter_NlinksFinal$from))
length(unique(filter_NlinksFinal$to))
length(filter_ref$original_seqID)

#curate node information
# node information will be used later to color code plot based on other typing methods
nodes <- arrange(unique(filter_NlinksFinal[1]), from)
node_info <- inner_join(nodes, filter_ref, by = c("from" = "original_seqID")) %>% select(from, Nterm, colors)

#exclude anything that is aligning to itself
exclude_self <- filter_NlinksFinal[!(filter_NlinksFinal$from == filter_NlinksFinal$to),]
exclude_self <- exclude_self %>% select(from, to, evalue)
...
Gambiense <- exclude_self[!(grepl("Rhodesiense", exclude_self$from)), ] %>% .[!(grepl("Rhodesiense", .$to)), ]
...
legend("bottomleft", bty = "n",
       ...
       col= color2[as.numeric(levels(gv2com$eigen_cluster))],pch = 19, pt.cex = 1.2, cex=0.8, horiz = FALSE)
