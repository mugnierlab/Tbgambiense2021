...
CN_df <- read.csv("FigureData/Supptable3_tbgVSGexpression.csv") %>%
  select(cdHit_ID, nterm_typesubtype, cterm_type) %>%
  distinct() %>%
...
#N-term on x axis
NxC <- read.csv("FigureData/Supptable3_tbgVSGexpression.csv") %>%
  select(cdHit_ID, nterm_typesubtype, cterm_type) %>%
  distinct() %>%
...
```{r}
#C-type by location
loc_C <- read.csv("FigureData/Supptable3_tbgVSGexpression.csv") %>%
  select("location", "patient_number", "cdHit_ID", "cterm_type") %>%
  distinct() %>%
