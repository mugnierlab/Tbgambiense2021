...
Cluster 82 is putative actin

Cluster 86 is NLI interacting factor-like phosphatase, putative

Cluster 0 is dynein heavy chain

Cluster 61 is RNA-editing complex protein MP42
...
```{r}
# create upset plot
plotdata <- read_csv("FigureData/Mulindwa2018_rhodesienseVSGtyped.csv")
...
tbr_list <- list(
  CSF58 = unique(as.character(plotdata$cluster[plotdata$patient_number =="58"])),
  CSF71 = unique(as.character(plotdata$cluster[plotdata$patient_number =="71" & plotdata$fluid == "CSF"])),
#  pA74 = unique(as.character(plotdata$cluster[plotdata$patient_number =="74"])),
  rRNA71 = unique(as.character(plotdata$cluster[plotdata$patient_number =="71" & plotdata$fluid == "blood"])),
  rRNA73 = unique(as.character(plotdata$cluster[plotdata$patient_number =="73"])),
  rRNA80 = unique(as.character(plotdata$cluster[plotdata$patient_number =="80"])),
  rRNA81 = unique(as.character(plotdata$cluster[plotdata$patient_number =="81"])))
