...
* Tb427 has recently been re-characterized and we used most up-to-date assembly and identified VSG sequences from the genome sequence using HMMscan
...
Fig5A <- plotEATRO %>% ggplot(aes(x = source, y = percent, fill = nterm_typesubtype)) +
  ...
  scale_fill_manual(values = c("skyblue", "steelblue1", "steelblue", "tomato", "red3")) + 
...
rhodesiense <- read_csv("FigureData/Mulindwa2018_rhodesienseVSGtyped.csv")
...
rhodesiense <- rhodesiense %>% select(cluster, nterm_typesubtype) %>% .[!duplicated(.$cluster), ]
...
gambiense <- read_csv("FigureData/TbgHighlyExpressed.csv")
# drop duplicates, pick out the types of each unique cluster
gambiense <- gambiense %>% select(cluster, nterm_typesubtype) %>% .[!duplicated(.$cluster), ]
...
Fig5B <- comparePatient %>% 
  ...
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("skyblue", "steelblue1", "steelblue", "tomato", "red3", "lightgrey")) + 
  scale_y_continuous(limits = c(0, 105)) +
  theme_classic() + 
  ylab("Percent VSG") + 
  labs(fill = "Subtype") +
